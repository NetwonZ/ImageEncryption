...
if __name__ == "__main__":
	...
	#sym:inspect_binary_file
	def view_seq(x0, z0, steps):
		start = 0
		end = start + steps
		seq = np.empty((end, cml.L), dtype=float)
		x = np.asarray(x0, dtype=float).copy()
		z = float(z0)
		for i in range(end):
			x, z = cml.step(x, z)
		...
		#show start to end state
		import matplotlib.pyplot as plt
		plt.figure(figsize=(10, 6))
		plt.plot(seq_flat[start:end], marker=".", linestyle="", markersize=1.0, alpha=0.9)
		plt.title(f"Salomon CML Sequence from step {start} to {end}")
		plt.xlabel("Index in Sequence")
		plt.ylabel("State Value")
		plt.grid(True, alpha=0.3)
		plt.tight_layout()
		plt.show()
	view_seq(x0, z0, steps=10000)
